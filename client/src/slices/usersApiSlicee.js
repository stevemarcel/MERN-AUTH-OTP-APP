...
export const usersApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // Delete Multiple Users by Admin request endpoint
    deleteUsersByAdmin: builder.mutation({
      query: (userIds) => ({
        // userIds will be an array of strings
        url: `${USERS_URL}`,
        method: "DELETE",
        body: { userIds }, // Send the array of IDs in the request body
      }),
      invalidatesTags: ["User"], // Invalidate tag to re-fetch user list after bulk delete
    }),
  }),
});
